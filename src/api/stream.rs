...
async fn prefetch_subtitles(
    ...
    let client = match opensubtitles_client(state).await {
        Ok(client) => client,
        Err(error) => {
            tracing::info!(session = %session_id, %error, "skipping auto-subtitles");
            return Vec::new();
        }
    };
    ...
    for language in &languages {
        ...
        let Some(result) = top else {
            tracing::info!(session = %session_id, %language, "no subtitle found");
            continue;
        };
        ...
        prefetched.push(PrefetchedSubtitle {
